		...
		t.Run(app, func(t *testing.T) {
			for _, tc := range []struct {
				...
				t.Run(tc.name, func(t *testing.T) {
					for _, name := range []string{"ADDRESS", "REPORT_INTERVAL", "POLL_INTERVAL", "RATE_LIMIT", "KEY", "STORE_INTERVAL", "FILE_STORAGE_PATH", "RESTORE", "DATABASE_DSN", "AUDIT_FILE", "AUDIT_URL", "CRYPTO_KEY"} {
						t.Setenv(name, "")
